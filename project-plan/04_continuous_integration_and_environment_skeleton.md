# Milestone 04 — Continuous Integration and Environment Skeleton
...
Establish automated verification and promotion rules for every component and environment.

## 2. Business reason

Manual-only validation cannot safely support billing, customer data, Android releases, or real businesses. CI must reject broken changes before they reach staging or production.
...
- Add CI workflows for Android, backend, web, migrations, and infrastructure validation.
- Add unit-test, lint, build, vulnerability, and secret-scanning jobs.
- Define development, staging, and production configuration boundaries.
- Publish versioned build artifacts.
...
- CI jobs must be deterministic and avoid hidden local dependencies.
- Production deployment must require explicit protected approval.
...
- Validate migrations against ephemeral PostgreSQL.
...
- Validate OpenAPI syntax and breaking changes when contracts appear.
...
- Build debug and release candidates; run unit and static analysis.
...
- Run format, validate, and plan checks for infrastructure code.

## 13. Security and privacy requirements

- Fail builds on exposed secrets and defined high-severity vulnerabilities, with documented exception process.
...
- Introduce intentional failures in each workspace and confirm CI rejects them.
...
- Configure repository environments and required reviewers.
...
- All workspaces are built and tested on every pull request.
- Broken migrations and secret leaks fail CI.
- Production promotion cannot occur from an unprotected branch.
...
- Artifact manifest
- Environment protection screenshots or notes
...
```text
You are implementing Milestone 4: Continuous Integration and Environment Skeleton for the Walk-In Queue Manager project.
