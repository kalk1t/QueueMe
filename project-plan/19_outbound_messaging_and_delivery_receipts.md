# Milestone 19 — Outbound Messaging and Delivery Receipts
...
Reliably enqueue, send, retry, track, and bill queue notification messages without duplicate logical notifications.

## 2. Business reason

Customers depend on timely notifications, while the business must pay only for traceable provider-billed segments and receive clear delivery diagnostics.
...
- Create outbound job pipeline.
- Trigger joined, almost-ready, your-turn, and no-show events.
- Implement idempotency, retries, provider callbacks, status mapping, segment finalization, and status-page fallback.
...
- Logical notification ID is unique per queue entry, event type, and transition version.
- Job retries must not create duplicate provider sends after uncertain outcomes without reconciliation.
...
- Message attempts, provider status history, logical notification key, final segment count, failure reason.
...
- Internal send request, delivery callback, message diagnostics.
...
- Show whether customer notification is pending, delivered, or failed where useful.

## 11. Web changes

- Owner/admin message detail.

## 12. Infrastructure and operations changes

- Queue worker scaling, dead-letter queue, alarms.

## 13. Security and privacy requirements

- Signed callback verification, content redaction, abuse controls.
...
- Provider timeout.
- Callback order permutations.
- Failed and undelivered states.
- Spending-limit denial.
...
- Send all four notification types to test phones.
...
- The same logical event is not sent twice.
- Delivery state is eventually consistent with provider callbacks.
- Final segments enter the usage ledger once.
...
```text
You are implementing Milestone 19: Outbound Messaging and Delivery Receipts for the Walk-In Queue Manager project.
